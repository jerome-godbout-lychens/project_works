...
// GetElementById retrieves an element by its identifier, checking cache first.
...
func (s *ElementService) GetElementById(ctx context.Context, elementId string) (*domain.Element, error) {
	cacheKey := fmt.Sprintf("element:%s", elementId)
...
func (s *ElementService) ListElementsByProject(
	...
	return s.elementStore.ListElementsByProject(ctx, projectId, filter)
}
...
func (s *ElementService) UpdateElement(ctx context.Context, elementId string, element *domain.Element) error {
	...
	if err := s.elementPendingChangeStore.UpsertPendingChange(ctx, elementId, snapshotMap); err != nil {
...
func (s *ElementService) DeleteElement(ctx context.Context, elementId string) error {
	// Delete pending change if exists (safe to call even if no pending change exists)
	_ = s.elementPendingChangeStore.DeletePendingChange(ctx, elementId)

	// Delete element from store
	if err := s.elementStore.DeleteElement(ctx, elementId); err != nil {
		return err
	}
...
func (s *ElementService) SearchElements(
	...
	return s.elementStore.SearchElements(ctx, projectId, query, limit, offset)
}
